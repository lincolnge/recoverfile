...
//		printf("DIR_Name %%u: \t%u\n", ptr2.DIR_Name);
		...
		fseek(fp, directoryArea+64+20*512, SEEK_SET);	
...
//		fwrite(&ptr2.DIR_Name[0], sizeof(ptr2.DIR_Name[0]), 1, fp);
		...
		//这一part给小文件用？ 貌似文件没内容的不需要EOF？ 
		fseek(fp, FatArea+4*3, 0);	//FatArea为16384, 4是cluster 4个字节，3是第3个cluster 
		fwrite(EOFValue, 4, 1, fp);
		
		//大文件的EOF
		fseek(fp, FatArea+4*5, 0);	//比如说 DIR_FstClusLO与 DIR_FstClusHI得到的值是5，文件大小为d21f， 就是1f2d（16） 
		// 1f2d(16) = 7981; 7981除以512 >= 15，
		//所以 循环15次 
		
		char FATValue[4];
		int i = 0;
		for(i=0; i <= 14; i++){	//需要一个循环，循环根据文件大小决定 
...
//		FstClus = 4;
		printf("%ld ", FstClus+1);
		fseek(fp, FatArea+FstClus, SEEK_SET);
		long Num = ptr2.DIR_FileSize/ptr.BPB_BytsPerSec;	//data的大小 除以512 就是循环次数 
		printf("Num %ld ", Num);
		FATVal =  FstClus + 1;
		for(i = 0; i <= Num-2; i++){
			fseek(fp, FatArea+4*(FstClus+1+i), 0);
			FATVal++;
			FATValue[0] = FATVal;
			if(FATValue[0] == 0x00)
				FATValue[1] += 1;
...
//			printf("%d ",i);
		...
		fseek(fp, FatArea+4*(FstClus+Num-1), 0);
		fwrite(EOFValue, 4, 1, fp);
		printf("what??2\n");
